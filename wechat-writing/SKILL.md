...
- **给出新主题、事件、现象、链接或研究问题，但尚未给出中心判断**：必须先读取 [选题与方向](references/ideation.md)，给出候选、推荐一项并等待选择。问题即使很具体，或使用了“为什么”“探讨一下”“写一篇”等措辞，也不等于方向已经确定。
- **明确要求跳过选题并直接成稿**：先在内部按 [选题与方向](references/ideation.md) 比较可能方向，再读取 [写作与成稿](references/drafting.md)；选择最可靠的一项继续，不展示候选、不暂停，把关键假设简短说明给用户。
- **已经给出可直接论证的中心判断，或选中了此前的方向**：读取 [写作与成稿](references/drafting.md)，不强制重新发散。
...
5. 完整成稿前完成内部审校。只向用户展示成稿和少量仍需作者决定的问题。
