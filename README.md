...
- **按需研究与材料准入**：只核实会改变结论、可信度或边界的事实；查到的资料不自动进入正文。
- **成稿**：围绕最短论证链组织材料，只展开一条主解释，重要内容讲透，次要内容点到为止。
- **删除式压缩**：每篇完整新稿和全文改稿都先删除无职责材料、重复观点、多余比喻和机器模式。
- **改稿与审校**：对旧稿先诊断再修改，对终稿做 brief、事实、论证、结构和语言检查。
- **风格学习**：从范文和作者本人的修改中提炼可复用的长期偏好。

## 这个 Skill 不做什么

- 不编造作者亲历事件、朋友与采访对象、对话、数据、引语、来源、日期地点和机构名；场景的氛围与感官细节可以为真实基础补写。
...
- **选择方向**：从主题种子中找出真正不同的探索路线，用户选中一项后不立即写作。
- **确认中心主题**：把暂定观点讨论成一句可论证、有边界的判断，获得用户明确确认后才继续。
- **最小 brief 与最短论证链**：锁定读者问题、核心判断、唯一主解释、最少推理步骤、竞争解释、边界和作者原文的密度基线。
- **材料准入与按需研究**：材料只有承担前提、证据、解释、反例或边界时才进入正文；研究结果是备选材料，不是正文配额。
- **写作**：围绕最短论证链组织材料；每节只承担一个主要推进，评论和分析只展开一条主解释。
- **删除式压缩**：先删除无职责材料、重复结论、同层例子和多余比喻，再调整仍有必要的句子；人味来自真实场景的质感，不来自无中生有的事件。
...
- 选择方向不等于确认中心主题；系统会继续与用户收敛最终判断，并再次等待确认。
- “直接写”只能跳过候选展示，不能跳过中心主题确认；同一条请求已明确给出最终判断并要求按它写时例外。
- 中心主题确认后才形成最小 brief 和最短论证链，研究和正文都必须服从它。
- 研究只在事实、时效性或风险需要时触发。
- 数据、案例和背景必须说明其论证职责；真实、有趣或已经查到都不是保留理由。
- 用户已经写出有效切入时，以其进入速度和信息密度为基线，只补关键推理、必要证据和边界。
- 事实检查会先核验来源和时效，再进入审校，不把核查自动扩成全文重写。
- 普通“润色一下”默认只改善表达并保留内容，不扩大成全文改稿。
- 完整新稿和全文改稿必须按“结构与内容 → 删除式压缩与自然化 → 终检”各执行一次。
...
| `research.md` | 事实密集、时效性强或高风险 | 什么需要核实，材料怎样通过准入门，证据不足时如何降级 |
| `argument.md` | 材料已有但论点松散 | 怎样搭最短论证链和唯一主解释，并检查材料职责、章节层级与旁枝 |
...
| `natural-editing.md` | 所有完整新稿和全文改稿；或局部要求去 AI 味 | 怎样先做文章级删除式压缩，再去除机器模式并保留作者声口 |
...
- 回答了读者真正的问题；
- 核心任务贯穿全文，争议性文章有最短论证链和唯一主解释；
- 每项数据、案例和背景都有明确的论证职责；
- 关键事实可追溯，或已明确降级；
- 相关的重要反方与适用边界得到处理；
- 没有虚构亲历事件、人物、对话、数据与来源，氛围感官细节只加在真实场景上；
- 标题、开头、章节和结尾各自承担实际任务；
- 同一手法没有在每一节重复到可预判，结尾只交付一个判断；
- 无职责材料、重复结论、同层弱例子和多余比喻已经删除；
- 语言自然，但没有改变事实与立场。
